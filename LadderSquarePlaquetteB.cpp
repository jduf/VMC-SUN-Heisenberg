...
LadderSquarePlaquetteB::LadderSquarePlaquetteB(System const& s, Vector<double> const& t):
	System(s),
	Ladder<double>(4,"ladder-squareplaquetteB"),
	t_(t)
{
	if(t_.size()==3){
		if(status_==2){
			...
			system_info_.text("LadderSquarePlaquetteB :");
...
void LadderSquarePlaquetteB::compute_H(){
	H_.set(n_,n_,0);
...
void LadderSquarePlaquetteB::create(){
	compute_H();
	diagonalize(true);
...
void LadderSquarePlaquetteB::save_param(IOFiles& w) const {
	if(w.is_binary()){
		std::string s("t=(");
...
void LadderSquarePlaquetteB::display_results(){
	compute_H();
	std::string t(my::tostring(t_));
	draw_lattice(true,true,true,"ladder-squareplaquetteB -d:t "+t, "t=("+t+")");
}

void LadderSquarePlaquetteB::check(){
	info_ = "";
	path_ = "";
	dir_  = "./";
	filename_ ="ladder-squareplaquetteB";
	display_results();
