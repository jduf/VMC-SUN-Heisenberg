...
void LadderRectangularPlaquetteC::display_results(){
	...
	if(rst_file_){
		...
		std::string run_cmd("./mc -s:wf ladder-rectangularplaquetteC");
		...
		rst_file_->figure(dir_+filename_+"-lr.png","long range correlations",RST::target(relative_path+filename_+"-lr.gp")+RST::scale("200"));
		rst_file_->figure(dir_+filename_+"-as.png","(anti)symmetric correlations",RST::target(relative_path+filename_+"-as.gp")+RST::scale("200"));
	}
}
