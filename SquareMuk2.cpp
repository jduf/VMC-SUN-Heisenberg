...
SquareMuk2::SquareMuk2(System const& s, double const& mu):
	System(s),
	...
	mu_(mu)
{
    ...
    } else { std::cerr<<__PRETTY_FUNCTION__<<" : N/m != 2"<<std::endl; }
}
...
void SquareMuk2::compute_H(unsigned int const& c){
	...
	for(unsigned int i(0);i<obs_[0].nlinks();i++){
		s0 = obs_[0](i,0);
		s1 = obs_[0](i,1);
                if(obs_[0](i,5)==1 && obs_[0](i,3)==1){ H_(s0,s1) = -2*(obs_[0](i,4)?bc_*t:t); }
