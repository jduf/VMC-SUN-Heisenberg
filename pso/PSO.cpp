...
unsigned int Particle::Nfreedom_ = 0;
Vector<double> Particle::min_ = Vector<double>();
Vector<double> Particle::max_ = Vector<double>();

void Particle::set(unsigned int const& Nfreedom, double const& cg, double const& cp){
	Nfreedom_ = Nfreedom;
	...
	if(cg+cp>=4){
		...
		std::cerr<<"Particle::set(unsigned int Nfreedom, double cg, double cp) : cg+cp<4 => chi=nan. Keep definition cg=cp=2.05, chi=0.729844"<<std::endl;
	}
