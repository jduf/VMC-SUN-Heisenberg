...
class Particle {
	public:
		Particle(unsigned int Nparam, double cg, double cp, double (*f)(std::vector<double>));
		~Particle();
...
Particle::Particle(unsigned int Nparam, double cg, double cp, double (*f)(std::vector<double>)):
	x_(Nparam),
	b_(Nparam),
	v_(Nparam),
	min_(Nparam),
...
void Particle::init(unsigned int thread){
...
void Particle::move(std::vector<double> gb){
	for(unsigned int i(0);i<x_.size();i++){
...
void Particle::set_limit(unsigned int param, double min, double max){
	min_[param] = min;
	max_[param] = max;
}
...
void Particle::result(){
	std::cerr<<"best particle ";
	for(unsigned int j(0);j<x_.size();j++){
		std::cerr<<b_[j]<<" ";
	}
	std::cerr<<" -> "<<f_(b_)<<std::endl;
}
#endif
