...
LadderSquarePlaquetteA::LadderSquarePlaquetteA(System const& s, Vector<double> const& t):
	System(s),
...
void LadderSquarePlaquetteA::compute_H(){
	H_.set(n_,n_,0);
...
void LadderSquarePlaquetteA::create(){
	compute_H();
	diagonalize(true);
...
void LadderSquarePlaquetteA::save_param(IOFiles& w) const {
	if(w.is_binary()){
		std::string s("t=(");
...
void LadderSquarePlaquetteA::display_results(){
	...
	if(rst_file_){
		...
		std::string run_cmd("./mc -s:wf ladder-squareplaquetteA");
...
void LadderSquarePlaquetteA::check(){
	...
	path_ = "";
	dir_  = "./";
	filename_ ="ladder-squareplaquetteA";
	display_results();
