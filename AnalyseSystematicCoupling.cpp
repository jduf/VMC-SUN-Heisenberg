		...
		case 7:
			{
				...
				while(kept_samples_J_.target_next() && display_results_){
					i++;
					kept_samples_J_.get().display_results(my::tostring(i)+"-",sim_,info_,analyse_,path_,dir_,&list_rst_.last(),false);
				}
				kept_samples_J_.set();
			}break;
		case 6:
			{
				...
				best_J_.set_target();
				while(best_J_.target_next() && display_results_){ best_J_.get().display_results("",sim_,info_,analyse_,path_,dir_,&list_rst_.last(),true); }
				best_J_.set();
