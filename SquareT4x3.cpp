...
SquareT4x3::SquareT4x3(System const& s, Vector<double> const& t):
	System(s),
	Square<double>(set_ab(),12,"square-T4x3"),
	t_(t)
{
	if(t_.size()==24){
		...
		if(status_==2){
			...
			system_info_.text("SquareT4x3 :");
			system_info_.item("Each color has the same Hamiltonian.");
			system_info_.item("12 sites in a 4x3 unit cell");
	...
	} else { std::cerr<<__PRETTY_FUNCTION__<<" : t must contain 24 values (currently contains "<<t_.size()<<")"<<std::endl; }
}

/*{method needed for running*/
void SquareT4x3::compute_H(){
	H_.set(n_,n_,0);
...
void SquareT4x3::save_param(IOFiles& w) const {
	if(w.is_binary()){
		std::string s("t=(");
...
Matrix<double> SquareT4x3::set_ab() const {
...
unsigned int SquareT4x3::unit_cell_index(Vector<double> const& x) const {
...
void SquareT4x3::display_results(){
	compute_H();
	draw_lattice(true);
...
void SquareT4x3::check(){
	info_ = "";
	path_ = "";
	dir_  = "./";
	filename_ ="square-T4x3";
	display_results();
